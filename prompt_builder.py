...
def build_call_context(
    ...
    if _is_marathi and (_product_persona or _product_call_flow):
        ...
        dynamic_context += (
            ...
            f"- WRONG: 'Amcha office No. 123, XYZ Building madhe ahe' (BANAVLA!)\n"
            f"- WRONG: 'Guntvanukit 50,000 te 2 lakh rupaye ahe' (BANAVLA! pricing mahit nahi tar deu nakos)\n"
            ...
            f"- BANNED FILLERS: 'चालू', 'बघा', 'चांगलं' ne response SURU KARU NAKOS. Direct answer de.\n"
            f"- WRONG: 'चालू, amchyakade AEPS, money transfer, bill payment, ticket booking, insurance, loan...' (list dump + chalu filler)\n"
            f"- RIGHT: 'Amchyakade AEPS, money transfer, insurance ashya services ahet.'\n"
            f"- List dyaychi tar TOP 3 FAKTA sang. Full list dump KARU NAKOS.\n"
            f"- Conversational Marathi examples:\n"
            f"  - 'ho na, tumhala interest ahe tar ek meeting fix karu ka?'\n"
            f"  - 'arey wah, mast! kadhi free ahat?'\n"
            f"  - 'nakki, ha business tumchya sathi fit ahe.'\n"
