...
async def test_poll_crm_leads(mock_up, mock_create, mock_get):
    ...
    with patch.object(HubSpotCRM, "fetch_new_leads", return_value=[{"external_id": "1", "first_name": "Test"}]):
        with patch.object(HubSpotCRM, "update_lead_status"):
            # We want to run poll_crm_leads for one cycle only
            # Since it's a while True, let's patch asyncio.sleep to raise an exception to break the loop
